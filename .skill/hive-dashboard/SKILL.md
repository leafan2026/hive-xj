...
- KV caches are `hive:stats:v9`, `hive:entries:v5`, `hive:weekly:v6`, `hive:loop:v1`, and `hive:meta:v1` through binding `CACHE`.
...
- Filtered `/api/dashboard` responses are memoised under `hive:q:v4:<meta.updatedAt>:<hash>`. The hash covers every key in `FILTER_KEYS` **and the resolved from/to dates**. If you add a filter parameter, add it to `FILTER_KEYS` too — otherwise two different filters share a cache entry and the dashboard serves the wrong numbers.
